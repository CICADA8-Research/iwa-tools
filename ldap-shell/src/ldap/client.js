...
export class LdapClient {
  ...
  async modify(dn, changes) {
    ...
    await this._writeMessage(this._nextId(), tlv(OP.MODIFY_REQUEST, concat([octetString(dn), sequence(...chSeq)])));
  ...
  async *search({ baseDN, scope = SCOPE.SUBTREE, filter: filt, attributes = [], pageSize = 1000 }) {
