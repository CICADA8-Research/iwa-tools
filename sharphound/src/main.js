...
const els = {
  ...
  collectOpts: $('collect-opts'), stealth: $('stealth'), excludeDCs: $('exclude-dcs'),
...
// ---- Collection method presets (matching original SharpHound) ----
const PRESETS = {
...
const ALL_METHODS = PRESETS.all;
const methodBoxes = () => document.querySelectorAll('[data-method]');

function applyPreset(name) {
  const set = new Set(PRESETS[name] || []);
  for (const cb of methodBoxes()) cb.checked = set.has(cb.dataset.method);
  syncPresetHighlight();
}

function syncPresetHighlight() {
  const current = new Set();
  for (const cb of methodBoxes()) if (cb.checked) current.add(cb.dataset.method);
  for (const btn of document.querySelectorAll('.preset')) {
    const set = new Set(PRESETS[btn.dataset.preset] || []);
    const match = set.size === current.size && [...set].every((m) => current.has(m));
...
for (const cb of methodBoxes()) {
  cb.addEventListener('change', syncPresetHighlight);
}
...
function syncUI() {
  ...
  els.queryOpts.classList.toggle('hide', m !== 'query');
  els.collectOpts.classList.toggle('hide', m !== 'collect');
...
function readConfig() {
  ...
  if (mode === 'collect') {
    for (const cb of methodBoxes()) if (cb.checked) methods.add(cb.dataset.method);
    if (methods.size === 0) throw new Error('Select at least one collection method.');
  }
  return {
    host,
    ...
    excludeDCs: els.excludeDCs.checked,
