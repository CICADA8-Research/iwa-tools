...
// MsvAvChannelBindings (AvId 0x000A) = MD5(gss_channel_bindings_struct) over the
// tls-server-end-point application data, spliced before MsvAvEOL so the NTProofStr
// binds to the TLS channel (WinRM HTTPS + channel-binding enforcement).
function withChannelBinding(targetInfo, appData) {
  ...
  av[0] = 0x0a; av[2] = 0x10;
  av.set(md5(concat([head, appData])), 4);
  const n = targetInfo.length;
  return concat([targetInfo.subarray(0, n - 4), av, targetInfo.subarray(n - 4)]);
}

export function buildAuthenticate({ user, domain, password, ntHash, type2, workstation = '', exportedSessionKey, channelBinding = null }) {
  const clientChallenge = randomClientChallenge();
  const timestamp = type2.timestamp || nowFiletime();
  const targetInfo = channelBinding ? withChannelBinding(type2.targetInfo, channelBinding) : type2.targetInfo;
  const resp = computeNtlmv2Response(
    user, domain, ntHash || password,   // ntHash (Uint8Array) takes the pass-the-hash path
    type2.serverChallenge, clientChallenge, timestamp, targetInfo,
  );
