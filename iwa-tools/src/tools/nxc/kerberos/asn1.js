...
export function kdcReqBody({
  ...
  sname, snameType = NAME_TYPE.SRV_INST, till, nonce, etypes,
  ...
  if (additionalTickets && additionalTickets.length) {
    parts.push(ctx(11, sequence(...additionalTickets)));
  }
  return sequence(...parts);
}
...
export function parseTicket(buf) {
  const { seqStart, seqEnd } = unwrapApp(buf);
  const m = tagMap(buf, seqStart, seqEnd);
  return {
    realm: strOf(buf, inner(buf, m[0xa1])),
    encPart: parseEncryptedData(buf, inner(buf, m[0xa3])),
  };
}
