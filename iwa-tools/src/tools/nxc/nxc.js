...
const MODULE_DOCS = {
  ...
  'smb:dcsync':        '[USER]  DRSUAPI replication — bare = dump every account (SAMR-enumerated), USER = only that user.',
  'smb:exec':          'CMD  Execute command via svcexec.',
  'smb:atexec':        'CMD  Execute command via Task Scheduler (TSCH) — reliable on hardened DCs where svcexec fails.',
  'smb:ntds':          'Download ntds.dit + SYSTEM hive via VSS shadow copy (parse with secretsdump.py locally).',
  ...
  'mssql:query':       'SQL  Run one SELECT / batch and print rows.',
  'mssql:dbs':         'List databases (sys.databases).',
  'mssql:users':       'List server principals (SQL logins + Windows).',
  'mssql:whoami':      'Current login / server / login class.',
  'mssql:sysinfo':     'Server version, edition, product level, host name.',
  'mssql:privesc':     'Find IMPERSONATE-able logins + sysadmin roster.',
  'mssql:tables':      '[DB]  List tables in a database (or master).',
  'mssql:columns':     'TABLE  List columns of a table.',
  'mssql:mssql-search':'PATTERN  Search table names for a keyword.',
  'mssql:logins':      'sys.server_principals rows with is_disabled flag.',
  'mssql:backups':     'Recent BACKUP DATABASE history.',
  'mssql:jobs':        'SQL Agent jobs.',
  'mssql:credentials': 'sys.credentials — mapped identities for external access.',
  'mssql:brute':       'USER FILE  Brute-force login (rate-limited).',
  // ---- FTP ----
