...
const AV_EOL = 0x0000;
const AV_NB_COMPUTER_NAME = 0x0001;
const AV_NB_DOMAIN_NAME = 0x0002;
const AV_DNS_COMPUTER_NAME = 0x0003;
const AV_DNS_DOMAIN_NAME = 0x0004;
const AV_DNS_TREE_NAME = 0x0005;
...
// NTOWFv2 = HMAC_MD5(MD4(UTF16LE(passwd)), UTF16LE(Uppercase(user) + domain)).
export function ntowfv2(user, domain, password) {
  const ntlmHash = md4(utf16le(password));
  return hmacMd5(ntlmHash, utf16le(user.toUpperCase() + domain));
}

// Compute the NTLMv2 and LMv2 responses given the pieces taken from the
// server CHALLENGE. `timestamp`, `serverChallenge`, `clientChallenge` are
// Uint8Arrays (8 bytes); `targetInfo` is the raw AV_PAIR block.
export function computeNtlmv2Response(
  user, domain, password, serverChallenge, clientChallenge, timestamp, targetInfo, ntHash,
) {
  const responseKey = ntHash
    ? hmacMd5(ntHash, utf16le(user.toUpperCase() + domain))
    : ntowfv2(user, domain, password);
  const temp = concat([
...
export function parseType2(buf) {
  ...
  while (pos + 4 <= targetInfo.length) {
    ...
    else if (id === AV_NB_COMPUTER_NAME) nbComputerName = utf16leDecode(targetInfo.slice(pos + 4, pos + 4 + len));
    else if (id === AV_NB_DOMAIN_NAME) nbDomainName = utf16leDecode(targetInfo.slice(pos + 4, pos + 4 + len));
    else if (id === AV_DNS_COMPUTER_NAME) dnsComputerName = utf16leDecode(targetInfo.slice(pos + 4, pos + 4 + len));
    else if (id === AV_DNS_DOMAIN_NAME) dnsDomainName = utf16leDecode(targetInfo.slice(pos + 4, pos + 4 + len));
    else if (id === AV_DNS_TREE_NAME) dnsTreeName = utf16leDecode(targetInfo.slice(pos + 4, pos + 4 + len));
    pos += 4 + len;
  }
  return { serverChallenge, flags, targetInfo, timestamp, nbComputerName, nbDomainName, dnsComputerName, dnsDomainName, dnsTreeName };
}
