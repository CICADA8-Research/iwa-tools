...
const BINARY = new Set([
  ...
  'msds-allowedtoactonbehalfofotheridentity', 'securityidentifier',
]);
  ...
  async connect(host, port, creds) {
    ...
    if (creds.authMethod === 'simple') {
      ...
      await kerberosSpnegoBind(this.ldap, {
        ...
        password: creds.password, hash: creds.hash || null, ticket: creds.ticket || null, channelBinding, log: this._log,
      });
    } else {
      const { user, domain } = parseIdentity(creds.user || creds.bindDN, creds.domain);
