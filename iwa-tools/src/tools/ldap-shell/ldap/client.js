...
import {
  TAG, tlv, concat, integer, enumerated, boolean, octetString, set,
...
export const filter = {
  ...
  not: (sub) => tlv(0xa2, sub),                                      // [2] NOT
  ge: (attr, value) => tlv(0xa5, concat([octetString(attr), octetString(value)])),    // [5] GE
  le: (attr, value) => tlv(0xa6, concat([octetString(attr), octetString(value)])),    // [6] LE
  extensible: (attr, value, rule, dn = false) => {                   // [9] extensibleMatch
    const p = [];
    if (rule) p.push(tlv(0x81, toBytes(rule)));
    if (attr) p.push(tlv(0x82, toBytes(attr)));
    p.push(tlv(0x83, toBytes(value)));
...
export class LdapClient {
  ...
  // ---- Write operations (RFC 4511) ----------------------------------------
  // Values may be strings (UTF-8) or Uint8Array (binary: unicodePwd, SDs, …).

  // attributes: { type: [value, …], … }
  async add(dn, attributes) {
    const attrSeq = Object.entries(attributes).map(([type, vals]) =>
      sequence(octetString(type), set(...vals.map((v) => octetString(v)))));
    await this._writeMessage(this._nextId(), tlv(OP.ADD_REQUEST, concat([octetString(dn), sequence(...attrSeq)])));
    this._checkResult(await this._readOpResult(), 'add', dn);
  }

  // changes: [{ op: 'add'|'delete'|'replace', type, values: [value, …] }]
  async modify(dn, changes, controls = null) {
    const OPN = { add: 0, delete: 1, replace: 2 };
    const chSeq = changes.map((c) =>
      sequence(enumerated(OPN[c.op]), sequence(octetString(c.type), set(...(c.values || []).map((v) => octetString(v))))));
    await this._writeMessage(this._nextId(), tlv(OP.MODIFY_REQUEST, concat([octetString(dn), sequence(...chSeq)])), controls);
    this._checkResult(await this._readOpResult(), 'modify', dn);
  }

  async delete(dn) {
    await this._writeMessage(this._nextId(), tlv(OP.DEL_REQUEST, toBytes(dn))); // primitive
    this._checkResult(await this._readOpResult(), 'delete', dn);
  }

  // RFC 4532 "Who am I?" — returns the authzid (e.g. "u:PK\\administrator").
  async whoami() {
    await this._writeMessage(this._nextId(), tlv(OP.EXTENDED_REQUEST, tlv(0x80, toBytes('1.3.6.1.4.1.4203.1.11.3'))));
    const msg = await this._readMessage();
    const kids = [...children(msg.buf, msg.op.valueStart, msg.op.valueEnd)];
    const code = readInt(msg.buf, kids[0].valueStart, kids[0].valueEnd);
    if (code !== 0) throw new LdapError(`whoami failed (code ${code})`, code);
    for (const k of kids) if (k.tag === 0x8b) return readString(msg.buf, k.valueStart, k.valueEnd); // responseValue [11]
    return '';
  }
  ...
  async _readOpResult() {
    ...
    const kids = [...children(msg.buf, msg.op.valueStart, msg.op.valueEnd)];
    return {
      code: readInt(msg.buf, kids[0].valueStart, kids[0].valueEnd),
      diag: kids[2] ? readString(msg.buf, kids[2].valueStart, kids[2].valueEnd) : '',
    };
  }

  _checkResult(r, op, dn) {
    if (r.code !== 0) throw new LdapError(`${op} "${dn}" failed (code ${r.code})${r.diag ? ': ' + r.diag : ''}`, r.code);
  }
  ...
  async *search({ baseDN, scope = SCOPE.SUBTREE, filter: filt, attributes = [], pageSize = 1000, controls: extraCtrls }) {
    ...
    do {
      ...
      const ctrlSeqs = [sequence(octetString(PAGED_OID), octetString(pagedValue))];
      if (extraCtrls) for (const c of extraCtrls) ctrlSeqs.push(c);
      const controls = tlv(0xa0, concat(ctrlSeqs));
