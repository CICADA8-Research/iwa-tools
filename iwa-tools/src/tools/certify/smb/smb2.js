...
export class Smb2Client {
  ...
  async _call(command, body, opts = {}) {
    ...
    let r = await this._read();
    while (r.status === 0x00000103) r = await this._read(); // skip async STATUS_PENDING interim responses
    return r;
  }
  ...
  async login({ user, domain, password }) {
    // buildAuthenticate throws on `domain: undefined`; treat missing domain as
    // empty so a caller that forgot -d gets a proper NTLM auth attempt with
    // an empty NetBIOS domain (server can still map local accounts / trust
    // NTLM without a domain hint).
    domain = domain || '';
  ...
  // One-way SMB2 WRITE to a pipe (for DCE-RPC AUTH3, which has no response).
  async writePipe(treeId, fileId, data) {
    const body = concat([
      u16(49), u16(0x70),              // StructureSize, DataOffset (header + 48-byte fixed body)
      u32(data.length),               // Length
      u64(0),                          // Offset
      fileId,                          // 16
      u32(0), u32(0),                  // Channel, RemainingBytes
      u16(0), u16(0),                  // WriteChannelInfoOffset, Length
      u32(0),                          // Flags
      data,
    ]);
