...
export function parseFilter(str) {
  let pos = 0;
  function parse() {
    if (str[pos] !== '(') throw new Error(`LDAP filter: expected ( at ${pos}`);
    pos++;
    let r;
    if (str[pos] === '&') { pos++; const s = []; while (str[pos] === '(') s.push(parse()); r = tlv(0xa0, concat(s)); }
    else if (str[pos] === '|') { pos++; const s = []; while (str[pos] === '(') s.push(parse()); r = tlv(0xa1, concat(s)); }
    else if (str[pos] === '!') { pos++; r = tlv(0xa2, parse()); }
    else r = parseItem();
    if (str[pos] !== ')') throw new Error(`LDAP filter: expected ) at ${pos}`);
    pos++;
    return r;
  ...
  function parseItem() {
    ...
    const attr = str.slice(s, pos);
    if (str[pos] === ':') return parseExtensible(attr);
    if (str[pos] === '>' && str[pos + 1] === '=') { pos += 2; return tlv(0xa5, concat([octetString(attr), octetString(readVal())])); }
    if (str[pos] === '<' && str[pos + 1] === '=') { pos += 2; return tlv(0xa6, concat([octetString(attr), octetString(readVal())])); }
    if (str[pos] === '~' && str[pos + 1] === '=') { pos += 2; return tlv(0xa8, concat([octetString(attr), octetString(readVal())])); }
    pos++;
    if (str[pos] === '*' && (pos + 1 >= str.length || str[pos + 1] === ')')) { pos++; return tlv(0x87, toBytes(attr)); }
    const val = readVal();
    if (val.includes('*')) return buildSub(attr, val);
    return tlv(0xa3, concat([octetString(attr), octetString(val)]));
  }
  function parseExtensible(attr) {
    pos++;
    const s = pos;
    while (pos < str.length - 1 && !(str[pos] === ':' && str[pos + 1] === '=')) pos++;
    const ext = str.slice(s, pos);
    pos += 2;
    const val = readVal();
    const segs = ext.split(':').filter(x => x);
    let dn = false, rule = '';
    for (const g of segs) { if (g.toLowerCase() === 'dn') dn = true; else rule = g; }
    const p = [];
    if (rule) p.push(tlv(0x81, toBytes(rule)));
    if (attr) p.push(tlv(0x82, toBytes(attr)));
    p.push(tlv(0x83, toBytes(val)));
    if (dn) p.push(tlv(0x84, Uint8Array.of(0xFF)));
    return tlv(0xa9, concat(p));
  }
  function buildSub(attr, val) {
    const parts = val.split('*');
    const subs = [];
    for (let i = 0; i < parts.length; i++) {
      if (!parts[i]) continue;
      subs.push(tlv(i === 0 ? 0x80 : i === parts.length - 1 ? 0x82 : 0x81, toBytes(parts[i])));
    }
    return tlv(0xa4, concat([octetString(attr), sequence(...subs)]));
  }
  function readVal() { const s = pos; while (pos < str.length && str[pos] !== ')') pos++; return str.slice(s, pos); }
  return parse();
}
...
export class LdapClient {
  ...
  async modify(dn, changes) {
    const opMap = { add: 0, delete: 1, replace: 2 };
    const changeSeqs = changes.map(c => {
      const vals = (c.values || []).map(v => typeof v === 'string' ? octetString(v) : octetString(v));
      return sequence(enumerated(opMap[c.op] || 0), sequence(octetString(c.attr), tlv(0x31, concat(vals))));
    });
    const req = tlv(OP.MODIFY_REQUEST, concat([octetString(dn), sequence(...changeSeqs)]));
    const id = this._nextId();
    await this._writeMessage(id, req);
    const msg = await this._readMessage();
    if (msg.op.tag !== OP.MODIFY_RESPONSE) throw new Error('expected modify response');
    const code = this._resultCode(msg);
    if (code !== 0) {
      const kids = [...children(msg.buf, msg.op.valueStart, msg.op.valueEnd)];
      const diag = kids[2] ? readString(msg.buf, kids[2].valueStart, kids[2].valueEnd) : '';
      throw new LdapError(`modify failed (code ${code})${diag ? ': ' + diag : ''}`, code);
    }
  }

  async add(dn, attributes) {
    const attrSeqs = attributes.map(a => {
      const vals = a.values.map(v => typeof v === 'string' ? octetString(v) : octetString(v));
      return sequence(octetString(a.name), tlv(0x31, concat(vals)));
    });
    const req = tlv(OP.ADD_REQUEST, concat([octetString(dn), sequence(...attrSeqs)]));
    const id = this._nextId();
    await this._writeMessage(id, req);
    const msg = await this._readMessage();
    if (msg.op.tag !== OP.ADD_RESPONSE) throw new Error('expected add response');
    const code = this._resultCode(msg);
    if (code !== 0) {
      const kids = [...children(msg.buf, msg.op.valueStart, msg.op.valueEnd)];
      const diag = kids[2] ? readString(msg.buf, kids[2].valueStart, kids[2].valueEnd) : '';
      throw new LdapError(`add failed (code ${code})${diag ? ': ' + diag : ''}`, code);
    }
  }

  async delete(dn) {
    const req = tlv(OP.DELETE_REQUEST, toBytes(dn));
    const id = this._nextId();
    await this._writeMessage(id, req);
    const msg = await this._readMessage();
    if (msg.op.tag !== OP.DELETE_RESPONSE) throw new Error('expected delete response');
    const code = this._resultCode(msg);
    if (code !== 0) {
      const kids = [...children(msg.buf, msg.op.valueStart, msg.op.valueEnd)];
      const diag = kids[2] ? readString(msg.buf, kids[2].valueStart, kids[2].valueEnd) : '';
      throw new LdapError(`delete failed (code ${code})${diag ? ': ' + diag : ''}`, code);
    }
  }
